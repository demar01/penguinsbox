...
```{r pluck-check}
 'Muy bien. pluck() hace lo mismo que pinguino_lista_Torgersen$masa_corporal_g and pinguino_lista_Torgersen[["masa_corporal_g"]], pero pluck() es más fácil
...
`pinguino_lista` es una lista con las medidas de pinguinos de las islas Torgersen, Biscoe y Dream.
...
```{r pinguino-lista-check}
 "Cada elemento de pinguino_lista tiene la misma estructura que pinguino_lista_Torgersen."
```
...
Tercero, debemos pasar la funcion a `map()`. 
...
```{r exp_lm-hint-1}
"Remember the two rules for map expressions: 1) begin with a ~, 2) refer to the elements with .x y 3) Pasa la funcion a map(). Do not surround the expression with quotes."
```

```{r exp_lm-solution}
pinguino_lista %>%
  map(~ lm(masa_corporal_g ~ anio, data = .x))
```
