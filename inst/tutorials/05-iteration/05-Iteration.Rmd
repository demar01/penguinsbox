...
```
Suppose you want to calculate the average each of the three variables in `penguin_measures`. You could calculate the average of each variable individually writing on the box below:
...
You can indeed calculate the mean of each variable in this way, but you would be repeating your code (remember the rule of thumb: never copy and paste your code more than twice). To avoid this, you could use  **a for loop**.
...
`penguins_list_Torgersen` is a list of penguin measurements from Torgersen Island. You can inspect this list in the exercise below.. 
...
Can you calculate the increase in `body_mass_g` from 2017 to 2019 in the list pinguino_list_Torgersen? This may seem like an easy task, and it will be the basis of our iteration exercise.

### Data wrangling

First you have to get `body_mass_g` values. This can be a bit tricky since the values are stored within the `penguins_list_Torgersen` list. 

You can extract the values with the `pluck()` function from the purrr's package.

### pluck() 

`pluck()` extracts an item from a list, by name or by position. Returns the content of the element as is, without surrounding it in a new list.

For example, `pluck()` extracts `body_mass_g` from `penguins_list_Torgersen`.
...
```{r lm-hint}
"Leave the code as is and click Submit reply."
```
...
```{r lm-check}
grade_code("lm function builds a linear model")
```
...
Now imagine that you want to make this model for each of the 3 islands included in `penguin_list`. This is not going to be very difficult
 because we have already done the most complicated thing, which is to write the model. However there are 3 rules that we must know.
 
### Rule 1 - ~

First, put `~` at the beginning of the expression. This alerts `map ()` that you are giving an expression to execute:

For example, `~lm(body_mass_g ~ year, data = penguins_list)`

### Rule 2 - .x

Second, replace the _name of the list to manipulate_ with `.x`
...
Third, give the function to map().

For example, `penguins_list %>% map(~lm(body_mass_g ~ year, data = .)`

###

Suppose you want to run a linear model to calculate body mass based on year and head size. I show you an example of how this model would be for the list `pinguino_lista_Torgersen`. Could you implement this model for the `penguin_list` data?
...
```{r exp_lm-solution}
penguins_list %>%
  map(~ lm(body_mass_g ~ year + cabeza, data = .x))
```
